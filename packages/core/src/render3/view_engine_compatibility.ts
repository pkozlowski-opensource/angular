...
import {appendChildView, createLContainer} from './instructions';
...
import {CONTEXT, EmbeddedViewFactoryInternal, LView, QUERIES, RENDERER} from './interfaces/view';
import {assertNodeOfPossibleTypes} from './node_assert';
import {appendChild, nativeInsertBefore, nativeNextSibling, nativeParentNode} from './node_manipulation';
import {getParentInjectorTNode} from './node_util';
import {getLView, getPreviousOrParentTNode} from './state';
import {getParentInjectorView, hasParentInjector} from './util/injector_utils';
import {findComponentView} from './util/view_traversal_utils';
import {getComponentViewByIndex, getNativeByTNode, isComponent, isLContainer, isRootView, lContainerToViewContainer, lViewToView, unwrapLContainer} from './util/view_utils';
import {getEmbeddedViewFactoryInternal, viewContainerGet, viewContainerInsertAfter, viewContainerRemoveInternal, viewDestroyInternal} from './view';
  ...
  if (!R3TemplateRef) {
    ...
    R3TemplateRef = class TemplateRef_<T> extends TemplateRefToken<T> implements IvyTemplateRef<T> {
      ...
      createEmbeddedView(context: T): viewEngine_EmbeddedViewRef<T> {
        ...
        return new ViewRef(this.embeddedViewFactory(context), context, -1);
      }
    };
  }
...
export function createContainerRef(
  ...
  if (!R3ViewContainerRef) {
    ...
    R3ViewContainerRef = class ViewContainerRef_ extends ViewContainerRefToken {
      ...
      constructor(
          private _lContainer:
              LContainer,  // TODO(misko): this should be ViewContainer (not LContainer)
  ...
  if (isLContainer(slotValue)) {
    ...
    // TODO(misko): All references to node_manipulation.ts should disappear.
