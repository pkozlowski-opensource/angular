 ...
 * in some cases (whitespaces can influence layout).
 */
export class WhitespaceVisitor implements html.Visitor {
  ...
  visitText(text: html.Text, context: any): any {
    ...
    if (!isBlank) {
      // lexer is replacing the &ngsp; pseudo-entity with \uE500
      return new html.Text(text.value.replace('\uE500', ' ').replace(/\s+/g, ' '), text.sourceSpan);
    }
