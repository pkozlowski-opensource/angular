  ...
  if (decl.components || decl.directives || decl.pipes) {
    ...
    for (const innerDecl of decl.declarations) {
      switch (innerDecl.kind) {
        case 'directive':
        case 'component':
          declarations.push(convertDirectiveDeclarationToMetadata(innerDecl));
          break;
        case 'pipe':
          declarations.push(convertPipeDeclarationToMetadata(innerDecl));
          break;
      }
    }
  }
...
function convertDeclarationFacadeToMetadata(declaration: R3TemplateDependencyFacade):
  ...
  return {
    ...declaration, type: new WrappedNodeExpr(declaration.type),
  }
}
