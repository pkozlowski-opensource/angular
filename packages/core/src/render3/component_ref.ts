...
import {LifecycleHooksFeature, createRootComponent, createRootComponentHost, createRootContext, resolveRootComponent} from './component';
import {getComponentDef} from './definition';
import {NodeInjector} from './di';
import {assignTViewNodeToLView, createComponentView, createLView, createTView, elementCreate, locateHostElement, refreshDescendantViews} from './instructions/shared';
import {ComponentDef} from './interfaces/definition';
import {TContainerNode, TElementContainerNode, TElementNode} from './interfaces/node';
import {RendererFactory3, domRendererFactory3, isProceduralRenderer} from './interfaces/renderer';
import {HEADER_OFFSET, LView, LViewFlags, TVIEW} from './interfaces/view';
import {enterView, leaveView, namespaceHTMLInternal, resetComponentState, setPreviousOrParentTNode} from './state';
...
export class ComponentFactory<T> extends viewEngine_ComponentFactory<T> {
  ...
  create(
    ...
    const hostTNode = createRootComponentHost(rootTView, projectableNodes);
    ...
    resolveRootComponent(rootLView, rootTView, hostTNode, this.componentDef.type);
    ...
    const componentRef = new ComponentRef(
        ...
        createElementRef(viewEngine_ElementRef, hostTNode, rootLView), rootLView, hostTNode);

    if (!rootSelectorOrNode || isIsolated) {
      // The host element of the internal or isolated root view is attached to the component's host
      // view node.
      componentRef.hostView._tViewNode !.child = hostTNode;
    }
    return componentRef;
  }
}
